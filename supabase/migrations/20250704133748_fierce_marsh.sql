...
CREATE UNIQUE INDEX IF NOT EXISTS teachers_user_email_contact_email_key 
ON teachers(user_email, contact_email);

-- Classes: Allow same class name/section across different users
CREATE UNIQUE INDEX IF NOT EXISTS classes_user_email_name_section_key 
ON classes(user_email, name, section);

-- Students: Allow same email across different users
CREATE UNIQUE INDEX IF NOT EXISTS students_user_email_contact_email_key 
ON students(user_email, contact_email);

-- Teacher Attendance: Allow same teacher/date across different users
CREATE UNIQUE INDEX IF NOT EXISTS teacher_attendance_user_email_teacher_date_key 
ON teacher_attendance(user_email, teacher_id, date);

-- Teacher Salaries: Allow same teacher/month across different users
CREATE UNIQUE INDEX IF NOT EXISTS teacher_salaries_user_email_teacher_month_key 
ON teacher_salaries(user_email, teacher_id, month_year);

-- Student Marks: Allow same student/exam across different users
CREATE UNIQUE INDEX IF NOT EXISTS student_marks_user_email_student_exam_key 
ON student_marks(user_email, student_id, exam_id);

-- Student Attendance: Allow same student/date across different users
CREATE UNIQUE INDEX IF NOT EXISTS student_attendance_user_email_student_date_key 
ON student_attendance(user_email, student_id, date);
